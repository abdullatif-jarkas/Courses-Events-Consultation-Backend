...
export const getAllInPersonCourses = asyncHandler(
  async (req: Request, res: Response) => {
...
export const getInPersonCourseById = asyncHandler(
  async (req: Request, res: Response) => {
...
export const deleteInPersonCourse = asyncHandler(
  async (req: Request, res: Response) => {
    const inPersonCourse = await InPersonCourse.findByIdAndDelete(
      req.params.id
    );
    if (!inPersonCourse) {
      res.status(404);
...
export const updateInPersonCourse = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params;
    const {
      title,
    ...
    if (Object.keys(courseUpdates).length > 0) {
      await Course.findByIdAndUpdate(inPersonCourse.courseId, courseUpdates, {
        new: true,
      });
    }
